...
def backup(pathname):
    ...
    try:
        ...
        print datetime.datetime.now(),'|',os.path.basename(pathname),'|','Moved to folder {0}'.format(os.path.basename(backup_path)),'|','nrt_cleanup'
    except Exception, e:
        print datetime.datetime.now(),'|',os.path.basename(pathname),'|','ERROR MOVING FILE','|','nrt_cleanup',str(e)
...
def cleanup():
    path = config.get('directories', 'nrt_out').rstrip('/')
    pathname = ""
    try:
                    ...
                    for pf in [i.strip(" ") for i in config.get('prefix', 'nrt_out_prefix').split(",")]:
                        if filename.startswith(pf):
                            ...
                            if os.stat(fullpath).st_mtime < (time.time() - (5*60)):
                                ...
                                print datetime.datetime.now(),'|',filename,'|','FTP Successful','|','nrt_cleanup'
                                backup(fullpath)
                            break
                    else:
                        print datetime.datetime.now(),'|',filename,'|','PREFIX not supported','|','nrt_cleanup'
    except Exception, e:
        print datetime.datetime.now(),'|',os.path.basename(pathname),'|','FTP Failed','|','nrt_cleanup',str(e)
